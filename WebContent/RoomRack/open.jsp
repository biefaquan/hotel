...
<script type="text/javascript">
	...
	function ChoseType(opentype) {
		...
		var rt_zdprice = document.getElementById("rt_zdprice").value;
		if(opentype.value=="天房"){
			Price.value=rt_price;
			ChangMoney();
		}else if(opentype.value=="钟点房"){
			Price.value=rt_zdprice;
			ChangMoney();
	...
	function Checked() {
		var start = "${nowdate.time }";
		var days = document.getElementById("Days").value;
		var startdate = "${rr.rr_startdate.time}";
		var id = "${rr.rr_state}";
		if((id==1||id==3)&&(startdate-start)/(24*3600*1000)<=days){
			alert("该时间段内已有预订订单！！");
			return false;
		}
		//验证姓名
...
<body>
    <!--入住弹出窗口-->
    <form id="myadd" action="/hotel/RegistrationInforController/add" target="_top" method="post" onsubmit="return Checked()">
    <div class="ruzhu_infor" style="width: 860px;">
        <div class="line">
            <div class="fl ordertitle">入住信息<c:if test="${room.r_state==2 }"><font color="red">(此房已有人预订，请与客户协商好)</font></c:if></div>
        ...
        <div class="types" style="background: rgb(239, 239, 239) none repeat scroll 0% 0%; border: 1px solid rgb(221, 221, 221);">
            <ul>
                <li>
                    ...
                    <input value="${room.rt.rt_name }" style="width: 132px; margin-right: 40px; display: inline; background: rgb(238, 238, 238) none repeat scroll 0% 0%;"/></li>
                    
                <li>
                    <label style="">房号：</label>
                    <input id="RoomNo" value="${room.r_roomnum }" readonly="readonly" style="width: 112px; margin-right: 40px; display: inline;background: rgb(238, 238, 238) none repeat scroll 0% 0%;" autocomplete="off" class="ac_input" type="text"/>
                    <input name="r.r_id" id="RoomId" value="${room.r_id }" type="hidden"/></li>
            ...
            <ul>
                ...
                <li>
                    ...
                    <select name="ri_keyway" id="OpenType" onchange="ChoseType(this)" style="width: 112px; margin-right: 27px; display: inline;">
                        ...
                        <c:if test="${room.rt.rt_zdstate==0 }">
                ...
                <li>
                	...
                	<input id="rt_zdprice" value="${room.rt.rt_zdprice }" type="hidden">
                </li>
                <li>
